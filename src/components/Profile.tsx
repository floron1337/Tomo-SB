...
const Profile = async ({ userId }: Props) => {
  ...
  const sentFriendReq = await db
    .select()
    .from(friends)
    .where(
      and(eq(friends.userId, currentUserId!), eq(friends.friendId, userId))
    );
  const receivedFriendReq = await db
    .select()
    .from(friends)
    .where(
      and(eq(friends.userId, userId), eq(friends.friendId, currentUserId!))
    );

  return (
    <div className="flex flex-col basis-2/3 max-md:basis-full gap-8 justify-start items-center py-20 h-full w-full overflow-y-scroll no-scrollbar text-white">
      ...
      {currentUserId === userId ? (
        <Button
          className="w-1/4 ml-auto mr-auto bg-secondary hover:bg-accent flex flex-row gap-1"
          asChild
        >
